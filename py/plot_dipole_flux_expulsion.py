#!/usr/bin/env python3
"""
Plot the superconducting-core flux-expulsion worked example (2c): a
perfectly diamagnetic sphere in a uniform ambient field, showing Br -> 0
inside and the induced-dipole exterior field, with the Btheta jump at the
surface (screening current).
...
Run from the project root, after generating the example data:
    ...
    ./build/dipole_flux_expulsion
    python3 py/plot_dipole_flux_expulsion.py
"""
...
EX_DIR = os.path.join(ROOT_DIR, 'examples', 'dipole_flux_expulsion')
R_SPHERE = 1.0
...
if __name__ == '__main__':
    ...
    for ax, blocks, ylabel, title in (
        (ax1, br_blocks, r'$B_r$', r'$B_r \to 0$ inside (flux excluded)'),
        (ax2, bth_blocks, r'$B_\theta$', r'$B_\theta$: screening-current jump at $r=R$'),
    ):
        style_axes(ax)
    ...
    fig.tight_layout()
    savefig_pair(fig, 'dipole_flux_expulsion_match')
