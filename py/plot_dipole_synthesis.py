...
if __name__ == '__main__':
    ...
    ax1.set_xlabel(r'$\theta$ (rad)', fontsize=10, color=INK_SECONDARY)
    ax1.set_ylabel(r'field (units of $\mu/R_0^3$)', fontsize=10,
                    color=INK_SECONDARY)
    ax1.set_title('Synthesized exterior dipole', color=INK_PRIMARY, fontsize=11)
    ax1.legend(frameon=False, fontsize=9, labelcolor=INK_SECONDARY)
    ...
    ax2.set_xlabel(r'$\theta$ (rad)', fontsize=10, color=INK_SECONDARY)
    ax2.set_ylabel('absolute error', fontsize=10, color=INK_SECONDARY)
    ...
    ax3.set_yscale('log')
    ax3.set_xticks(x)
    ax3.set_xticklabels([f'{int(l)}' for l in l_vals])
    ax3.set_xlabel(r'$\ell$', fontsize=10, color=INK_SECONDARY)
    ax3.set_ylabel('|overlap with target field|', fontsize=10,
                    color=INK_SECONDARY)
    ax3.set_title('Mode purity: only $\\ell=1$ survives', color=INK_PRIMARY,
                  fontsize=11)
    ax3.legend(frameon=False, fontsize=9, labelcolor=INK_SECONDARY)
